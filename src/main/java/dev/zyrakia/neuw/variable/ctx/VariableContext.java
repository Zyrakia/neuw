...
public class VariableContext {
     ...
     * 
     * @param descriptors the variable descriptors that this context
     * instantiates
     * @throws IllegalStateException if there are multiple descriptors with the
     * same identifiers
     */
    public VariableContext(Set<Variable<?>> descriptors)
     ...
     * @throws IllegalArgumentException if the given identifier does not exist
     * in this variable context
     ...
     * @throws UnsetRequiredVariableException if any of the variables do not
     * have a valid value set, but are required
     */
    public Map<String, Object> evaluate()
            throws UnsetRequiredVariableException {
        Map<String, Object> values = new HashMap<>();

        for (Map.Entry<String, Variable<?>.Instance> entry : this.instances
                .entrySet()) {
     ...
     * @throws ValidationException if any of the values returned by the
     * populator did not match the required type of the variable
     */
     ...
     * identifier. If no valid value is set, it will return {@code null}, unless
     * the variable is required, in which case it will raise the
     * {@link UnsetRequiredVariableException}.
     * 
     * @return the current value of this variable
     * @throws UnsetRequiredVariableException if the variable is required, but
     * no valid value is set
     * @throws IllegalArgumentException if the given identifier is not
     * associated with a variable
     */
    public Object evaluate(String identifier)
            throws UnsetRequiredVariableException, IllegalArgumentException {
        Variable<?>.Instance inst = this.getInstance(identifier);
        return inst.evaluate();
    }

    /**
     * Clears the current instances map, and instantiates all registered
     * descriptors as new instances.
     * 
     * @throws IllegalStateException if two variables with the same identifier
     * are instantiated
     */
    private void instantiate() throws IllegalStateException {
        ...
        for (Variable<?> descriptor : this.descriptors) {
            if (this.instances.containsKey(descriptor.identifier()))
                throw new IllegalStateException("The identifier \""
                        + descriptor.identifier() + "\" is already set.");
     ...
     * associated with a variable
     */
    private Variable<?>.Instance getInstance(String identifier)
        ...
        throw new IllegalArgumentException("The identifier \"" + identifier
                + "\" does not exist in this variable context.");
    }
